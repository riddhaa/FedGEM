...
G = 5
n_sample_vec = [500,2500,4500,6500]
runtime_AFCL = np.zeros(len(n_sample_vec))
runtime_k_FED = np.zeros(len(n_sample_vec))
runtime_FedKmeans = np.zeros(len(n_sample_vec))
runtime_FFCM1 = np.zeros(len(n_sample_vec))
runtime_FFCM2 = np.zeros(len(n_sample_vec))
runtime_GMM = np.zeros(len(n_sample_vec))
runtime_DpGMM = np.zeros(len(n_sample_vec))
runtime_kmeans = np.zeros(len(n_sample_vec))
runtime_clem = np.zeros(len(n_sample_vec))
    
for i in range(len(n_sample_vec)):
    n_samples = n_sample_vec[i]
    n_features = 15
    n_train_all = np.ones(G)*n_samples
...
exp_results = {'runtime_AFCL':runtime_AFCL,
               ...
               'n_sample_vec':n_sample_vec
                }

filename = 'FedClem_Scal_samp_'+str(int(timeit.default_timer()))+str(np.random.randint(0,high=1000))+'.mat'
