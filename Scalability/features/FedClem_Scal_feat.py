...
class FedClem:
    ...
    def comp_M_theta(self,theta_all,pi_all,local_data):
        ...
        for k in range(K):
            ...
            for n in range(N):
                gamma_i_k[n] = (pi_all[k]*rvs['rv_'+str(k)].pdf(local_data[n]))/np.max([denom_vec[n],1e-12])
    ...
    def rad_feasibility_prob(self,theta_all,pi_all,M_theta_k,eps,local_data,k):
        ...
        sum_gamma = 0
        for n in range(N):
            sum_gamma += (pi_k*rvs['rv_'+str(k)].pdf(local_data[n]))/np.max([denom_vec[n],1e-12])
            
            gamma_i_k =  (pi_k*rvs['rv_'+str(k)].pdf(local_data[n]))/np.max([denom_vec[n],1e-12])
...
n_test = 1000
n_features_vec = [5,25,45,65]

runtime_AFCL = np.zeros(len(n_features_vec))
runtime_k_FED = np.zeros(len(n_features_vec))
runtime_FedKmeans = np.zeros(len(n_features_vec))
runtime_FFCM1 = np.zeros(len(n_features_vec))
runtime_FFCM2 = np.zeros(len(n_features_vec))
runtime_GMM = np.zeros(len(n_features_vec))
runtime_DpGMM = np.zeros(len(n_features_vec))
runtime_kmeans = np.zeros(len(n_features_vec))
runtime_clem = np.zeros(len(n_features_vec))
...
for i in range(len(n_features_vec)):
    n_features = n_features_vec[i]
...
filename = 'FedClem_Scal_feat_'+str(int(timeit.default_timer()))+str(np.random.randint(0,high=1000))+'.mat'
