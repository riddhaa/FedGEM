...
n_test = 1000
n_clusters_vec = [5,25,45,65]

runtime_AFCL = np.zeros(len(n_clusters_vec))
runtime_k_FED = np.zeros(len(n_clusters_vec))
runtime_FedKmeans = np.zeros(len(n_clusters_vec))
runtime_FFCM1 = np.zeros(len(n_clusters_vec))
runtime_FFCM2 = np.zeros(len(n_clusters_vec))
runtime_GMM = np.zeros(len(n_clusters_vec))
runtime_DpGMM = np.zeros(len(n_clusters_vec))
runtime_kmeans = np.zeros(len(n_clusters_vec))
runtime_clem = np.zeros(len(n_clusters_vec))

n_features = 15
for i in range(len(n_clusters_vec)):
    n_clusters = n_clusters_vec[i]
...
filename = 'FedClem_Scal_clus_'+str(int(timeit.default_timer()))+str(np.random.randint(0,high=1000))+'.mat'
