...
def gen_data(perc_train, G, clusters_per_client=-1):
    ...
    x_all = X[:,1:-1].astype(float)
    y_all_raw = y1
    classes, y_all = np.unique(y_all_raw, return_inverse=True)
...
filename = 'FedClem_FrogB_'+str(int(timeit.default_timer()))+str(np.random.randint(0,high=1000))+'.mat'
