...
def gen_data(perc_train, G, clusters_per_client=-1):
    ...
    x_all_raw = np.asarray(X)[:,1:]
    y_all_raw = np.asarray(y)[:,0] - 1
    ...
    y_all[y_all_raw == 8] = 3
    y_all[y_all_raw == 9] = 4
    y_all[y_all_raw == 10] = 5
    y_all[y_all_raw == 11] = 6
    y_all[y_all_raw == 12] = 6
    y_all[y_all_raw > 12] = 7
...
filename = 'FedClem_Abalone_'+str(int(timeit.default_timer()))+str(np.random.randint(0,high=1000))+'.mat'
